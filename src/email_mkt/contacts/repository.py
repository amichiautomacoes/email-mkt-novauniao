...
class ContactRepository:
    ...
    def fetch_recipients(self, campaign_key: str, limit: int | None = None) -> list[dict]:
        filters = ContactFilters(campaign_key=campaign_key, limit=limit)
...
def _fetch_contacts(
    ...
    select_columns = [
        ...
        _select_column(columns, "nome", "nome", fallback_names=("name", "first_name", "primeiro_nome")),
    ...
    if lote_key is not None:
        ...
        where_clauses.append(
            sql.SQL("lower(regexp_replace(source.{}::text, '[^a-zA-Z0-9]', '', 'g')) = %s").format(
                sql.Identifier("lote")
            )
        )
    ...
    query = sql.SQL(
        ...
        table=sql.SQL("{}.{}").format(sql.Identifier(schema), sql.Identifier(CONTACTS_TABLE)),
...
def _select_column(
    ...
    for name in (preferred_name, *fallback_names):
        if name in columns:
            return sql.SQL("source.{}::text as {}").format(sql.Identifier(name), sql.Identifier(alias))
